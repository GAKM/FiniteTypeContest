...
def set_payoffs(group: Group):
    ...
    for p in players:
        if p == winner1:
            p.is_winner1 = True
            p.is_winner2 = False
            p.payoff = C.ENDOWMENT + C.THREE_PLAYER_LGN_PRIZE_1 - p.bid * p.participant.COST
        elif p == winner2:
            p.is_winner2 = True
            p.is_winner1 = False
            p.payoff = C.ENDOWMENT + C.THREE_PLAYER_LGN_PRIZE_2 - p.bid * p.participant.COST
