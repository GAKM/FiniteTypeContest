...
doc = """
Introduction page and comprehension checks for the Finite-Type Contest game
"""
...
class Player(BasePlayer):
    ...
    password = models.StringField(
        label="<i>ADMINISTRATOR USE ONLY.</i><br> Enter the access key:"#,
...
class Quiz2(Page):
    ...
    def error_message(player: Player, values):
        ...
        if errors:
            player.num_failed_attempts += 1
            if player.num_failed_attempts >= 6:
                player.failed_too_many = True
                # we don't return any error here; just let the user proceed to the
                # next page, but the next page is the 'failed' page that boots them
                # from the experiment.
...
page_sequence = [Intro, Quiz, Failed, Quiz2]
